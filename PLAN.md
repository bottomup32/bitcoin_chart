...
| **7. 메모리 서브시스템** | 계측(llm_calls) → 단기메모리 → as-of 수정 + 장기메모리 → 지식 코퍼스 → 지식 주입·귀속 → source_weights 학습 → 뉴스 | 7장 참조 |
...
에이전트 호출은 무상태였다. 어제 뭐라고 했는지, 그 예측이 맞았는지, 어떤 투자
원칙을 따르는지 아무것도 프롬프트에 들어가지 않았고, `run_evaluate`의 채점 결과는
오케스트레이터의 투표 가중치로만 쓰였다. 학습 루프가 절반만 닫혀 있었다.

### 설계 원칙 3가지

1. **"에이전트가 공부한다"는 불가능하다.** 파인튜닝이 아닌 이상 축적되는 이해는
   없고, 레버는 검색-후-프롬프트 주입뿐이다. 게다가 모델은 버핏·그레이엄·멍거·
   마크스를 이미 안다. 코퍼스의 가치는 (a) 어떤 원칙을 **이 사용자가** 채택했는지
   고정, (b) 오늘의 구체적 숫자에 강제 적용, (c) `source_weights`가 학습할 귀속
   채널 생성. 따라서 청크는 백과사전 요약이 아니라 **짧고 단정적인 의사결정 규칙**.
2. **Reflection은 이미 코드로 존재한다.** `run_evaluate`가 매일의 "예측이 맞았나"
   검사다. 빠진 건 그 결과를 다음 프롬프트로 되돌리는 것(단기메모리)과 누적 결과를
   증류하는 것(장기메모리)뿐 — v1에서 Reflection에 필요한 LLM 호출은 **0건**.
3. **에이전트에게 Brier·skill·weight를 절대 보여주지 않는다.** Brier는 에이전트가
   *정확*하려 할 때만 proper scoring rule이다. 지표를 보여주면 최적화 대상이 되고,
   모델의 순진한 대응(confidence를 0.5로 수축)은 `conflict_rules`가 투표 가중에
   쓰는 신호를 파괴한다. **결과(outcome)는 보여주고 점수(score)는 숨긴다.**

### 4종 메모리

| | 저장소 | 생성 | 프롬프트 비용 |
|---|---|---|---|
| **단기** | 기존 테이블 쿼리 (신규 테이블 없음) | 없음 | ~250 tok |
| **장기** | `agent_lessons` (append-only) | 템플릿, LLM 0건 | ~150 tok |
| **지식** | `knowledge_docs`/`knowledge_chunks` | 로컬 CLI 1회성 | ~300 tok |
| **뉴스** | `news_items` | RSS, 매일 | ~200 tok |

- **단기**: `agent_opinions` + `orchestrator_decisions` + `sim_evaluations` 조인.
  columns+rows 인코딩에 세션 오프셋(ISO 날짜 아님)으로 행당 ~15토큰.
- **장기**: 통계는 코드, 본문은 템플릿. 증거 게이트 `n_eff >= 30`은
  `run_evaluate.MIN_N_EFF`와 **같은 상수**(테스트로 강제). 미달이면 아무것도 쓰지
  않는다 — 침묵이 "데이터 부족" 문자열을 매일 지불하는 것보다 싸고 정직하다.
- **지식**: 검색은 **태그 매칭**. pgvector 기각(Anthropic 임베딩 API 없음 → 두 번째
  벤더·키·비용을 50~300청크 근사 최근접에 쓸 이유 없음), FTS 기각(트리거가 숫자
  상태라 질의 문자열이 없음). 모든 태그는 `indicators.py`/`wash_sale.py`가 이미
  계산하는 값에 대응 → 검색이 결정론적·리플레이 가능.
- **뉴스**: 웹 검색 도구가 아니라 RSS인 이유는 **재현성**이다. 검색 결과는 저장·
  재생이 안 되어 과거 세션 리플레이가 깨지고 무엇을 봤는지 감사할 수 없다. RSS는
  `published_at`이 있어 다른 모든 메모리와 같은 as-of 규칙으로 걸린다.

### 룩어헤드 방어

모든 학습 상태가 `as_of_trade_date`를 갖는다. `latest_agent_weights`에는 as-of
필터가 **아예 없었고**, 이는 `FORCE_ADVISE=1` 리플레이 시 오늘의 가중치가 과거
세션 결정에 들어가는 실제 버그였다(0004에서 수정).

- `run_date < D` **엄격히** (같음 아님)
- `eval_trade_date <= D`, 그리고 **`LEFT JOIN ... ON` 절에** — `WHERE`에 두면
  조용히 inner join이 되어 미해결 호출(=열린 포지션)을 전부 떨궈낸다
- `sim_evaluations.created_at`은 감사용일 뿐 **검색 필터로 쓰지 않는다** — 백필된
  평가는 created_at이 늦고 eval_trade_date가 이르다
- `agent_lessons`에 `superseded_at` **없음** — "언제 낡았다고 알게 됐는가"는 과거
  run_date 기준 미래 사실이다. as-of 정렬이 공짜로 supersede 의미를 준다
- `assert_as_of()` 런타임 가드 (DB 테스트가 없으므로 테스트 전용 불변식으로 부족)

### 알려진 한계 (정직하게)

- **source_weights 학습은 몇 달간 통계적으로 무의미하다.** 종목 2개, 일 ~2개 채점
  가능 의견이면 소스당 `n_eff=30`까지 ~75세션 ≈ 4개월. 그때까지 `shrunk_skill`은
  정확히 `credibility_prior`를 반환하고, 검색은 학습 가중치를 **무시한다**. 배관을
  깔되 루프는 닫지 않는다.
- **`risk`·`tax`는 Brier 채점 제외**이므로 거기 보여준 지식은 영원히 평가 불가.
  방향 채점을 억지로 붙여 신호를 만들지 않는다 — 4장이 막으려던 실수다.
- **앵커링이 최대 위험.** 단기메모리의 목표는 일관성인데, 그것을 달성하는 실패
  모드가 "새 증거에 갱신을 멈추고 어제를 반복"이다. 다른 모든 지표에서 성공처럼
  보이는 유일한 실패 모드라 `flip_rate` 진단을 따로 둔다. 0으로 붕괴하면 반응성을
  일관성의 외양과 맞바꾼 것.
- **출력 토큰이 진짜 위험.** 메모리는 싼 쪽(입력)이지만 길어진 rationale은 5배
  쪽이다. `llm_cost_daily`의 `output_tokens`를 감시한다.
